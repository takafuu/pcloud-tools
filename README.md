...
- `pushd gate` and `diffd gate` mark read-only gate diagnostics as not requiring routine operator verification; human checks are reserved for first real target review, real execution gate implementation, or actual pCloud/rclone transfer
...
- `transfer real-gate` exposes machine-readable real execution readiness (`blocked-final-review`, `blocked-approval`, or `blocked-implementation`) plus `real execution can run: no`
...
- `pushd transfer real-run` and `diffd transfer real-run` are hard refusals; even with `--execute` and any gate-like env values they return an error, show `real execution can run: no`, show that real/fake gate env values were not honored, write no state, and point back to `transfer real-gate`

...
- `scripts/pcloud-shadow-validation.py` runs a temp-dev-state shadow validation pass over preview, dry-run, action, and safety-guard paths without touching live state or pCloud remotes; it covers the fswatch resident gate using a temp fake `fswatch` discovered through `command -v` and the pCloud API long-poll gate without making API calls
...
- Remaining gates are fswatch resident implementation/start, pCloud API long-poll implementation/start, launchd/autosync integration, normal sync/resync migration validation, and old monolith archive; fswatch resident and pCloud API long-poll now have read-only gate checklists but are still not runnable

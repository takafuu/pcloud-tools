...
- `pushd transfer real-run` and `diffd transfer real-run` are hard refusals; even with `--execute` and any gate-like env values they return an error, show that real/fake gate env values were not honored, write no state, and point back to `transfer real-gate`

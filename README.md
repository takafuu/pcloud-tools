...
- Completed live checks now include bounded fswatch resident start, live pCloud API `/diff` one-shot, public normal sync migration validation, public autosync re-enable, and old monolith archive execution. Continuous daemon operation, launchd changes, and additional real transfer validation are optional future work and still require explicit operator gates and `--execute`

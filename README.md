...
- abnormal Discord notifications are optional and off by default. `pcloud-manager notify status|enable|disable|test` controls `PCLOUD_TOOLS_CHAT_NOTIFY_ENABLED`; when enabled, conflicts and transfer timeouts/failures notify via `PCLOUD_TOOLS_CHAT_NOTIFY_CMD`. Deferred manual-review records remain visible in status/xbar without recurring chat notifications
...
./pcloud-manager-dev legacy old-monolith-gate
...
- public `pcloud-manager` now invokes the Python implementation; launchd, fswatch, pCloud API polling, real transfer IO, and old monolith legacy archive are still unchanged

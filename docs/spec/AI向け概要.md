...
- `pushd transfer executor-run` / `diffd transfer executor-run` は dev-state fake-rclone 専用の queue executor tick。`pushd executor-run --execute` は tick 冒頭で missing-local upload record に `missing_since` を付け、TTL (`PCLOUD_TOOLS_PUSHD_MISSING_LOCAL_PRUNE_TTL_SECONDS`, default 600 秒) を過ぎた record だけ自動 prune してから plan を組む。`--execute --consume-on-success` で fake-rclone transfer と successful record consume まで閉じる。manual-review record がある場合は fake-rclone 開始前に拒否する。
